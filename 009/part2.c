...
static int prologue(struct solutionCtrlBlock_t *_blk, int argc, char *argv[])
{
    ...
    for (size_t ii = 1; ii < 10; ii++)
    {
        ...
        aoc_2d_object_h ntail = eng_obj_create(_ctx->_eng, fullname, &start, name, OBJ_PROPERTY_NO_COLLISION | OBJ_PROPERTY_NO_COLLISION);
...
error:
    ...
    else
    {
        ...
        dll_free_all(&_ctx->_tails, eng_obj_free);
    }
    free(_blk->_data);
    return ret;
}
